...
def show_only(ifnames):
    ...
    return filtered_current_state(base_filter_state)


def filtered_current_state(desired_state):
    """
    Given the desired state, return a filtered version of the current state
    which includes only entities that have been mentioned in the desired state.
    In case there are no interfaces for filtering, all are reported
    """
